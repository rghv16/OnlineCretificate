...
def board_contains_word_in_column(board, word):
    ...
    True
    >>> board_contains_word_in_column([['A', 'N'], ['T', 'T'], ['D', 'O'], ['R', 'Y']] , 'XO' )
    False
    """
    for index in range(len(board[0])):
        if word in make_str_from_column(board, index):
            return True
...
def num_words_on_board(board, words):
    ...
    cnt = 0
    for ele in words:
        if board_contains_word(board, ele):
            cnt += 1
    return cnt
...
def read_words(words_file):
    ...
    """
    return [ele.strip() for ele in words_file]
...
def read_board(board_file):
    ...
    """
    l = [ele.strip() for ele in board_file]
    p = []
    for ele in l:
        x = []
        for e in ele:
